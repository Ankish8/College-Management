  ...
  Object.keys(rawData).forEach(sheetLabel => {
    ...
    if (sheetLabel === 'B-Des UX 3 (Foundation)') {
      ...
      filteredStudents = sheetData.rawData.filter(student => {
        ...
        const isUXStream = student['Stream'] && 
          student['Stream'].toLowerCase().includes('ui') || 
          student['Stream'].toLowerCase().includes('ux');
      ...
      const streams = [...new Set(sheetData.rawData.map(s => s['Stream']).filter(Boolean))];
      streams.forEach(stream => {
        const count = sheetData.rawData.filter(s => s['Stream'] === stream && s['Registered/Not Registered'] === 'Registered').length;
    ...
    const cleanedStudents = filteredStudents.map(student => {
      const cleaned = {
        ...
        studentId: student[' Reg No./ Student ID NO']?.trim(),
        rollNumber: student['Roll No']?.trim(),
        name: student['Name of Student']?.trim(),
        contactNo: student['Student Contact No.'],
        personalEmail: student['Student Personal Email ID']?.trim(),
        officialEmail: student['Student JLU Official Email ID']?.trim(),
        registrationStatus: student['Registered/          Not Registered'] || student['Registered/Not Registered'],
        stream: student['Stream'] || `Bachelor of Design UX - Semester ${sheetData.semester}`
      };
