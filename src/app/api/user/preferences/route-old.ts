...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (error.code === 'P2021') {
        console.log('user_preferences table does not exist, using defaults')
        return NextResponse.json({
          viewModes: defaultPreferences.viewModes,
          updatedAt: new Date().toISOString(),
        })
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!user) {
      console.log("User not found for PUT, returning default preferences")
      // Return default preferences instead of 404 for missing users
      return NextResponse.json({
        viewModes: defaultPreferences.viewModes,
        updatedAt: new Date().toISOString(),
      })
    ...
    try {
      currentPreferences = await db.userPreferences.findUnique({
        where: { userId: user.id },
        select: { viewModes: true }
      })
    } catch (error: any) {
      if (error.code === 'P2021') {
        console.log('user_preferences table does not exist for PUT, using defaults')
        return NextResponse.json({
          viewModes: defaultPreferences.viewModes,
          updatedAt: new Date().toISOString(),
        })
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!user) {
      console.log("User not found for PATCH, returning default preferences")
      // Return default preferences instead of 404 for missing users
      return NextResponse.json({
        viewModes: defaultPreferences.viewModes,
        updatedAt: new Date().toISOString(),
    ...
    try {
      ...
      if (error.code === 'P2021') {
        console.log('user_preferences table does not exist for PATCH, using defaults')
        return NextResponse.json({
          viewModes: defaultPreferences.viewModes,
          updatedAt: new Date().toISOString(),
        })
